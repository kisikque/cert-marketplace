...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    ...
    return services.filter((s) =>
      (s.tags || []).some((t) => t.slug === activeTag)
    );
  }, [services, activeTag]);
  ...
  if (!res.ok && res.reason === "DIFFERENT_PROVIDER") {
    const ok = confirm(
      "В корзине услуги другого провайдера. Очистить корзину и добавить эту услугу?"
    );
    if (!ok) return;
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
        {tags.map((t) => (
          <button
            key={t.id}
            onClick={() =>
              setActiveTag((prev) => (prev === t.slug ? "" : t.slug))
            }
            style={{
              borderRadius: 999,
              padding: "4px 10px",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
          {filtered.map((s) => (
            <div key={s.id} style={{ border: "1px solid #ddd", borderRadius: 10, padding: 12 }}>
              <div style={{ fontSize: 12, opacity: 0.7 }}>{s.providerName}</div>
              <div style={{ fontWeight: 700 }}>{s.title}</div>
              <div style={{ fontSize: 13, marginTop: 6 }}>{s.description}</div>
              <div style={{ marginTop: 8, fontSize: 13 }}>
                <b>от {s.priceFrom ?? "—"} ₽</b> • срок от {s.etaDaysFrom ?? "—"} дн.
              </div>
              ...
              <button style={{ marginTop: 10 }} onClick={() => addToCart(s)}>
                В корзину
              </button>
            </div>
          ))}
